...
class MessagesUrTranslationTest {
    ...
    fun `urdu translation differs from english for key`(key: String) {
        val en = SliderMessages.get(key, "en")
        val ur = SliderMessages.get(key, "ur")
        assertNotEquals(en, ur, "Key '$key' should be translated to Urdu, not a copy of English")
    }
}