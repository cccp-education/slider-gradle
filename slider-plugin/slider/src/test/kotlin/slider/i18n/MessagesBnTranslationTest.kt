...
class MessagesBnTranslationTest {
    ...
    fun `bengali translation differs from english for key`(key: String) {
        val en = SliderMessages.get(key, "en")
        val bn = SliderMessages.get(key, "bn")
        assertNotEquals(en, bn, "Key '$key' should be translated to Bengali, not a copy of English")
    }
}