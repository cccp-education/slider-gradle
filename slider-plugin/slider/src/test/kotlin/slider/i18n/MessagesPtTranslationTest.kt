...
class MessagesPtTranslationTest {
    ...
    fun `portuguese translation differs from english for key`(key: String) {
        val en = SliderMessages.get(key, "en")
        val pt = SliderMessages.get(key, "pt")
        assertNotEquals(en, pt, "Key '$key' should be translated to Portuguese, not a copy of English")
    }
}