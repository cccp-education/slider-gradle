...
class MessagesHiTranslationTest {
    ...
    fun `hindi translation differs from english for key`(key: String) {
        val en = SliderMessages.get(key, "en")
        val hi = SliderMessages.get(key, "hi")
        assertNotEquals(en, hi, "Key '$key' should be translated to Hindi, not a copy of English")
    }
}